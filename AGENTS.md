...
# 进容器用的逃生口
docker compose exec server fobe-server admin unblock <ip|all> | reset-password | list-sessions --revoke
```
...
7. **AI 执行默认放行，但元操作强制确认**（面板密码、主密钥、AI 自身配置、**由 AI 发起的** agent 自更新——design §12.3）；每条执行写 `audit_logs`；AI 执行的约束是**有界闸**（每轮变更预算 / 每分钟上限 / 连续失败暂停 / 重复检测）与 `ai.default_policy=confirm`——**不要**再加回"一键冻结全部 AI 执行"的 Kill Switch：这个助手的能力就是跑命令，冻成只读等于让它失效（2026-09-18 拍板移除，见 design §12.3）。探针跟随服务端由 `agent.auto_update` 单独控制。§5.5 的**系统自动跟随**不属于元操作（服务端版本变更触发，AI 侧只有只读查询）。
   **2026-09-18 起 AI 的工具集只剩三个终端工具**（`run_shell` / `send_keys` / `read_terminal`；sing-box 四个 + `tail_logs` 已删，design §12.7），三处别记错：① **元操作强制确认在 AI 侧已无载体**（`install_singbox` 是最后一个元操作）——该规则仍约束 HTTP 接口那侧的动作，但**别再声称"AI 想给自己扩权时一定被拦"**；② 有界闸里的**"连续失败暂停"基本失效**（exit code 多半来自哨兵解析，拿不到即"未知"，而未知不算失败），别把它算进兜底；③ 新轴：连续 `read_terminal` 超 10 次即结束回合（`turn_end.reason=read_limit`）。
8. **agent 不调用外部命令做采集**（OpenWrt 是 busybox），只读 `/proc`、`/sys`、`statfs`。
9. **流量计数器 `delta < 0` 判为回绕/重启**：不计负数、重设基准，并记 `counter_reset` 事件。
10. **sing-box 变更走三道闸门**：下载+sha256 → `check` → 启动 → 30s 观察（进程存活 + 端口可连 + TLS 握手）→ 失败回滚 `.prev`。版本必须显式指定，不追 latest。**例外（2026-09-18）**：这条只约束**受管路径**（面板 / HTTP 接口触发的安装与生命周期）。AI 的工具集里已无 sing-box 工具，但它能在 `run_shell` 里手搓下载、覆盖二进制、重启——**那条路绕过全部三道闸门与回滚**。所以"删掉 AI 的 sing-box 工具"**没有**取缔这种操作，只是让 AI 不再有一条安全的路（design §12.7.6）。
...
- 新设置项用 `域.键` 命名（如 `server.public_url`、`ai.default_policy`），经 `SetSetting`/`GetSetting`（敏感的用加密版本）。
- 日志用 `log/slog`。
...
- **入站凭据（anytls 密码 / vless UUID）属于入站，不是全局设置（2026-09-17e 重写）**：旧文档那条"全局 `settings.anytls_password` + AI 工具 `set_anytls_password` + `Server.anytlsMu`"**已整个不存在**（`anytls_password.go`、`ensureAnytlsPassword`、`anytlsMu` 全无，`PUT /api/settings` 不接受该键），别再照它实现。现行真相在 `internal/server/httpapi/node_password.go`：**只有"面板创建这个入站"这条路径会生成**（`ensureNodeProxyPassword` → `singbox.GenerateAnytlsPassword`，16 位 `[A-Za-z0-9]`；vless 生成 v4 UUID）；其余所有调用方（启动时模板同步、改端口、重装、渲染订阅的兜底）都必须**读回来**（`nodeProxyPassword`，查找顺序 = `singbox_config:<node_id>` → 探针上报的 `local_config` → §19.9 的 `password_override` 兜底）。老节点零迁移：它们的密码本来就烤在自己的 `config.json` 与面板存的那份文档里。三处别改回去：①**密文存在但解不开**（换过主密钥、恢复了旧库）**绝不能当"没有凭据"**去新生成——`ensureNodeProxyPassword` 先验两个来源的**可读性**，读不出来就报错、什么都不动（`TestUnreadableReportIsNotMintedOver`），否则等于在操作员修密钥的当口静默轮换、把还在用旧密码的客户端全部打断；②**只有"创建入站"会 mint**，配置同步与订阅渲染一 mint 就是静默轮换；③字符集留在 `[A-Za-z0-9]`：同一个串要原样进 JSON 字段、第三方客户端的 URI auth 位与 Clash YAML 标量，只有字母数字在三种上下文里都不用转义。生成动作记 `anytls_password_generated` 审计（actor=`system`、带 `node_id`、值 `[redacted]`）。
...
- **`web/src/api.ts` 的 `norm*` 是白名单，新字段忘了加 = 静默吞掉（2026-09-19 踩过）**：`normSubscriptionEntry` 这类函数逐字段重建对象（因为服务端空值走 `omitempty`，缺键会让受控输入与 `t()` 拿到 `undefined`），所以**新增 wire 字段的顺序必须是 `types.ts` → `api.ts` 的 norm → 页面**；漏掉中间一步的症状是"后端 JSON 明明有值（直接 `curl` 可见），页面上永远是默认值/短横线"，且 `tsc` 全绿——接口里该字段是 optional，不写不会报错。排查"字段到不了页面"先看 norm，再看浏览器缓存。
