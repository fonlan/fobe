...
| `FOBE_VERSION` | `compose` | **仅本地构建**生效：注入 server/agent 的版本号。默认值故意不带数字（非发布形态），本地构建不会触发探针跟随；想演练就用真实版本号 |
...
- **前提是「服务端有个像版本号的版本」**：CI 发布的镜像注入 tag 号（去掉 `v` 前缀）；`scripts/dev.sh` 会自己造一个（内容寻址的 `dev-<哈希>`，见「本地开发」一节）。手工 `go run ./cmd/server` 没有 `-ldflags`、版本是 `dev`，本地 compose 不设 `FOBE_VERSION` 直接 `--build` 时是 `compose`——这两个**不是发布形态**，服务端不会下发目标（面板会写明原因）。
