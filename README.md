...
- **主密钥**：不设 `FOBE_MASTER_KEY` 时，入口脚本自动生成随机密钥落盘 `data/.master_key`（0600）并复用；**换了密钥，已加密的 AI key / Bot Token 全部失效，需要重填**。密文认的是这把密钥、**与机器无关**：带上 `data/.master_key` 整卷搬到别的机器照常解开；只搬数据库/导出文件、却让新机器自己生成了一把新密钥，才是解不开的那种情况（搬迁 = 搬 key）。
...
- **管探针上已有的 sing-box（和 one-sing.sh 共存）**：设置 → 服务器 → 编辑 → 「sing-box 服务端」卡片里，只要探针上有一份 sing-box，就会出现「**本机已有的 sing-box**」区块，并带一个入站表格：版本、服务状态、配置文件路径，以及 `/etc/one-sing/config.json` 里的每一个入站（协议 · 端口 · 状态 · tag · 凭据）；状态 Tile 此时会显示「**已识别（本机实例）**」——「未安装」只属于面板自己装的那套（点「安装 / 更新」才会改变它；它**以探针上报的那份文件为基底合并**——其他协议的入站（VLESS/Socks/SS）与手改过的字段原样保留，面板只保证自己的 anytls 入站在场，不会整份重写你的配置文件，也**按文件取端口**，不会把已经搬过的监听拉回旧端口、更不会顺手换掉密码）。**这份文件就是真相**——面板显示的就是探针盘上那一份，你不改它、它就不会动。入站**不需要"接管"**：探针一上报，全部监听都会以同等规则进入订阅；没有某一条被认定成「本节点入口」或享有特殊编辑限制。
  ...
  - **改端口是一次「搬移」**：保存后表格里立刻能看到结果——**新端口作为一行「添加中」出现**，被改的那行显示「**删除中**」（此刻探针上仍是旧端口在服务）。探针应用并重报后，两行合成一行「运行中」的新端口，不需要手动刷新页面。探针离线时这个状态会一直保留，它表示"改动还没落到探针上"。在这段窗口里**再改同一行**（例如改 tag）等于取消这次搬移：文档又回到旧端口，那行恢复成「运行中」。
...
- **端口转发（nftables）**：设置 → 服务器 → 编辑 → 「端口转发（nftables）」卡片里增删改探针上的转发规则。布局与 [nfpf.sh](https://github.com/fonlan/nfpf) 完全相同（`table ip nat` 的 `prerouting` DNAT + `postrouting` masquerade），所以**用那个脚本或手工 `nft` 加过的规则也会出现在这里，并且能在这里改**。列表默认读最近一次上报（探针离线也能看；同一份清单也在**节点详情页底部只读列出**，那里没有编辑按钮、也不会为打开页面去问探针），点「从探针刷新」会立刻问一次探针；改动是在线探针一次往返内生效，离线则入队（10 分钟内探针上线后自动执行）。首次添加会自动建表建链、打开内核 IPv4 转发，并把规则写进 `/etc/nftables.conf`（与 nfpf.sh 相同的持久化文件——**手工维护过该文件的注意，它会被实况覆盖**）。备注（comment）会写进规则本身，位置与 nfpf.sh 相同；双引号在 nft 字符串语法里没有转义写法，因此备注不允许含双引号（反斜杠按原样保存）。**若保存后备注为空**，说明链路里还有旧二进制（面板后端或探针 agent 是加备注字段之前的构建，Go 会静默忽略未知字段）：重启后端（`scripts/dev.sh` 起的话必须重启，后端不热更）、并让探针更新到当前 agent 后重存一次——现在这种情况会在卡片上给出明确提示，不再静默。
...
- **中转入口（A 转发到 B 的 anytls 端口）**：订阅绑定的单位是**入口**而不是节点——**一台服务器上的一条入站 = 一个入口 = 选择器里的一行**（同一台机器跑两条 anytls 入站就是两行、两个出站，各自勾选、各自命名；这类节点的自动名会带上端口，如 `东京:8443`，只有一条入站的节点则保持原名不加后缀）。如果某台服务器 A 的 nftables 转发正好指向 B 的 anytls 入站端口，这条订阅里就会出现第三个入口 **「B 经 A」**：客户端连 A 的 `IP:转发端口`，TLS 仍然终结在 B，所以证书 pin 的仍是 B 的；**落地节点 B 必须是一个可用的 anytls 节点**（装好 sing-box、上报了证书与主 IP——面板装的或 one-sing.sh 装的都行，判定读的就是探针上报的那份配置文件），跳板 A 自己不需要装 sing-box（它只转发包），但要有主 IP。B 有多条 anytls 入站时，中转出站 pin 的是**转发规则指向的那条**入站的证书。选择器沿旧规矩**只列能出现在订阅输出里的入口**；已绑定但当前不可渲染的入口仍会灰显列出（并给出原因），以便你取消勾选——**在面板上改端口，绑定会跟着走**；如果是在探针上直接改 `config.json` 换掉的端口，那一行会灰显标「配置里已经没有这个入站端口了」，重新勾选新端口即可。展开订阅的节点选择器可以看到 A 直连、B 直连、B 经 A 三条，各自勾选、各自命名（名称留空则用自动名：直连用节点名／「订阅名称」，中转默认 `目标 · 跳板:端口`，格式可在同一页的「中转入口」卡片里改）。新发现的中转入口默认**自动勾选**；取消勾选会记住（不会被自动加回来），要整体关掉自动纳入就关那张卡片里的开关。名字（含「编辑服务器 → 订阅名称」）就是客户端里的节点名与模板里 `fobe-<名字>` 的 tag，改名后模板里 pin 过旧 tag 的分组会失配，重拉订阅即可。
- **备份**：整机状态就是 `data/` 一个目录（数据库、主密钥、GeoIP 库、产物缓存、快照），停服后拷走即可。快照默认开启（启动即拍 + 每 24h 一轮 + 迁移前必拍，保留 3 份）；恢复 = 停服后用快照替换 `data/fobe.db`。面板里另有「导出/导入 JSON」：**自 2026-09-18 起导出文件携带密文凭据**（通知渠道的 Token/Secret、AI provider 的密钥与自定义请求头，均为 AES-GCM 密文），所以**导出文件等同于凭据文件，要和 `data/.master_key` 一样保管**；导入时会逐条试解密，**解不开的条目会在结果里列名**（说明目标面板的主密钥不是同一把），不会静默当成「没配过」。
- **登录失败自锁自救**：`docker compose exec server fobe-server admin unblock all`。
- **配 AI 助手**：设置 → **AI 助手**（`/settings/ai`）。可以加**多个 provider**，每个选一种协议（`openai-completions` / `openai-responses` / `anthropic-messages`）、填 root 语义的 base_url（表单会实时显示最终拼出的 endpoint）、API key（只写不回显）与可选的额外请求头；选一个 models.dev 的 provider slug 可预填协议与 base_url，但两者都能手改（slug 没有 `api` 字段时会留空，超纲协议如 azure/vertex 会强制你手选）；**中转站建议留空**——一个 provider 里就是多家厂商的模型，没有哪个 slug 代表得了它，留空时按模型 id 匹配元数据。模型可以**从上游拉取列表后勾选批量添加**，也可以手填 id；添加时会自动从 models.dev 匹配显示名、输入输出能力、思考档位、最大上下文与最大输出（同一个 id 被多家收录时取多数派的值，匹配结果会告诉你是哪一家），**任何一个字段手改后就固定下来**（后续刷新不再覆盖）。同一个模型挂到多个 provider 上仍是一行。「默认模型」卡片里选的 `(provider, 模型)` 对与**默认思考档位**决定新会话的起点（**档位下拉只列所选模型真正支持的档位**，模型没有任何可调档位时用文字说明，不会给你一个空下拉）：终端页 AI 侧栏的**发送行**里可以直接选模型（按 provider 分组）与思考档位（含关闭思考；模型不提供可调档位时会明说），两个下拉打开时就落在你配的默认值上——思考档位只在所选模型确实提供该档位时才预选它，其余情况留「默认(不指定)」由服务端按默认档位补。AI 助手会在一轮里连续执行多步（查→改→重启→验），回合进行中发送按钮会变成「停止」（同一个按钮，点一下就中止）；有风险的动作仍会弹确认，确认后接着跑。**注意**：这一版**不再有**旧的单 provider「AI Base URL / 模型 / API Key」设置项，升级后需要在新的 AI 页重新配一次。
- **常用命令（终端页右栏）**：设置 → **常用命令**（`/settings/commands`）维护命令片段（名称 + 命令，支持多行，可上移/下移排序，最多 50 条）。节点终端页的右侧栏是一个标签容器：「AI 助手」与「常用命令」两个标签，只渲染配置了的那侧；**点击命令会输入到左侧终端**——单行命令输入并立即执行，多行命令以粘贴方式插入（可检查，回车才执行；shell 不支持 bracketed paste 时多行点击被禁用并提示，避免逐行误执行）。命令保存在服务端、所有节点的终端通用，输入等同手敲、不写审计。
- **配通知**：设置 → 通知。三个渠道同级并列（Telegram / 飞书 / 通用 Webhook），各带启用开关与「发送测试消息」；飞书点「扫码接入」用飞书 App 扫码并在确认页创建应用即完成绑定，告警由该机器人私聊发给你（面板只出网、不需要公网回调地址），不想建应用就用群自定义机器人 Webhook。下方事件开关按类型决定推不推（探针上线下线、面板登录失败、流量阈值、缴费到期、sing-box 异常、更新异常、计数器重置），流量阈值也在这一页。关掉开关只停推送、不清配置，也不补推关闭期间的历史。探针的上/下线与每次实际投递都会写进**设置 → 审计日志**（`node_online` / `node_offline` / `notify_sent` / `notify_failed`，发起者显示为 `system`）：下线记录静默了多久，上线记录探针上报的版本（链路闪断自愈、连接没断过的探针由心跳带回在线，这类上线没有版本号，只记静默时长）；被开关拦下的告警不写——那是有意的静默。**审计日志保留最近 30 天**（设置 → 审计日志）：由后台定时任务自动清理，窗口比指标明细的 7 天长；需要长期留证的请按期自行导出；面板默认每页 20 条（下拉可改 20 / 50 / 100 / 200），底部有页码与「跳至第 N 页」。**注意**：审计行随时在追加（探针上线下线、通知投递），新记录插在列表顶部，翻页窗口会随它轻微移动——同一页码两次打开看到的行可能差一两条。**推送正文的语言（中文 / English）与显示时区（IANA 名，默认 UTC）也在这页切换**：面板界面语言是浏览器本地的、两者互相独立，这里只影响 Telegram / 飞书收到的文字（通用 Webhook 的 JSON 不受影响）；「发送测试消息」同样按当前设置渲染，改完下一条告警即生效。
