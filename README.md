...
- **配 AI 助手**：设置 → **AI 助手**（`/settings/ai`）。可以加**多个 provider**，每个选一种协议（`openai-completions` / `openai-responses` / `anthropic-messages`）、填 root 语义的 base_url（表单会实时显示最终拼出的 endpoint）、API key（只写不回显）与可选的额外请求头；选一个 models.dev 的 provider slug 可预填协议与 base_url，但两者都能手改（slug 没有 `api` 字段时会留空，超纲协议如 azure/vertex 会强制你手选）；**中转站建议留空**——一个 provider 里就是多家厂商的模型，没有哪个 slug 代表得了它，留空时按模型 id 匹配元数据。模型可以**从上游拉取列表后勾选批量添加**，也可以手填 id；添加时会自动从 models.dev 匹配显示名、输入输出能力、思考档位、最大上下文与最大输出（同一个 id 被多家收录时取多数派的值，匹配结果会告诉你是哪一家），**任何一个字段手改后就固定下来**（后续刷新不再覆盖）。同一个模型挂到多个 provider 上仍是一行。终端页的 AI 侧栏顶部可以选择模型（按 provider 分组）与思考档位（含关闭思考；模型不提供可调档位时会明说）。AI 助手会在一轮里连续执行多步（查→改→重启→验），随时可以点「停止」中止；有风险的动作仍会弹确认，确认后接着跑。**注意**：这一版**不再有**旧的单 provider「AI Base URL / 模型 / API Key」设置项，升级后需要在新的 AI 页重新配一次。
