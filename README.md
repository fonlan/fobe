...
- **接管探针上已有的 sing-box（one-sing.sh 装的也算）**：设置 → 服务器 → 编辑 → 「sing-box 服务端」卡片里，只要探针上已经有一份 sing-box，就会出现「**本机已有的 sing-box**」区块——版本、服务状态、配置文件路径，以及文件里的入站清单（类型 · 端口 · 有无凭据；fobe 不接管的协议会标出来而不是藏起来）。勾选要接管的入站，点「**接管勾选的入站**」即把它写进面板的期望状态：**端口与凭据原样保留**（脚本 anytls 的端口会成为面板入站端口，它的密码继续生效，客户端不用改配置），其余入站（VLESS Reality / SS2022 / Socks5）随每次下发一起写回 `config.json`，并从此刻起由面板管理、可被订阅引用。前提是服务端缓存里有可用版本（设置 → sing-box 里先下载一个；探针当前跑的那个版本若已在缓存里则优先沿用，不重下载）。**没勾选的入站会在下一次下发时被移除**——这就是「接管」的含义。接管后别再让 one-sing.sh 往那份 `config.json` 里加协议（加 C 会被下一次收敛覆盖），要加就在面板接管后由面板管。
