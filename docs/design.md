...
| 15 | 缴费周期 | 周期类型（无 / 按月 / 按天 / 按年，2026-09-15 增按年）+ 周期长度 + 下次到期日 + 费用，手动改；**周期长度单位随类型（天/月/年），类型只作记账口径、不参与任何到期计算**；**无续费按钮、无历史**。费用即 `node_billing.note`（2026-09-16 由「缴费备注」更名），自由文本、只展示：编辑页可填，概览卡片贴成标签 | 查不到"上期什么时候交的" |
...
> - **手工加的东西立刻可见**：订阅从"最近上报的文件"渲染——`jq` 加一个 VLESS，下一轮上报后订阅里就有它，不需要任何"接管"。`POST /api/nodes/{id}/singbox/refresh` 让操作员改完文件不用等一轮。`node_singbox.port` 只表示"哪个监听是面板自己的"（订阅条目名不带后缀、证书按上报值 pin），首次编辑时按文件里最后一个 anytls 入站确定，之后不再变（**2026-09-17d**：探针一上报就自动认定，不必先编辑一次；凭据改为按文件渲染，不再用全局密码覆盖）。
...
> **实现修订 2026-09-17d（自动接管：本机已有的监听就是本节点的，不需要操作员挑）**：17b 把「接管」降级成一次编辑之后，面板上仍留着一个勾选框加一个「接管勾选的入站」按钮，而它要做的事完全可以从探针上报的文件里读出来。本修订删掉这个动作：
> - **`node_singbox.port`（"哪个监听是本节点自己的"）在探针上报时自动认定**（`hub.recognizeLocalInboundPort`）：取文件里**最后一个** anytls 入站（one-sing.sh 往数组尾部追加，最后那个就是操作员最近设的）。只在节点还没有 port 时认定，之后**永不移动**——客户端按那个条目 pin 证书，挪动它等于悄悄换掉客户端连上的是什么；只认面板自己能管的端口段（10000–60000），脚本的 443 之类照常渲染（用自己的凭据、带 `协议:端口` 后缀），但面板绝不声明一个它装不上去的端口；文件里没有 anytls 就不认定，节点没有"自己的入站"，全部入站都带后缀。
> - **纯账面记账，探针一点不动**：认定只写 `node_singbox.port`，不写配置、不下发期望帧、不重启。这不是口头保证：`ListSingboxTargets` 要求 `desired_version`、`buildDesiredState` 要求版本或 config hash，只设 port 两条都不成立，`TestRecordSingboxLocalRecognizesTheNodesOwnInbound` 盯着这两条。
> - **凭据与证书都按文件渲染**：`LiveProxyNodes` 不再把面板的全局 anytls 密码写到那个端口上——文件是探针此刻真正在服务的东西，脚本创建的监听必须继续用它原来的密码，否则订阅会发出一条认证不了的 URI。面板写的那个监听，其密码由服务端在创建时现生成并写进文件，此后文件带着它，渲染照旧（面板装的节点行为不变）；**"全局密码"这个东西本身已在 §10.1 实现修订 2026-09-17e 里取消**。证书同理：文件里该端口的 PEM 优先，`node_singbox.cert_pem` 只在文件没给时兜底——探针换过证书之后，旧的那份不能再被 pin。
> - **删掉动作本身**：`POST /api/nodes/{id}/singbox/adopt` 与编辑器请求里的 `adopt` 标志一并删除（前端早已不用前者），i18n 里那批"接管"文案同时清掉。要改入站就在表格里改（读-合并-写），或在探针上改完点「从探针刷新」；入站表格给被认定的那个监听加一枚「本节点入口」标记。
> - **`extra_inbounds` / `BuildNodeConfigWithInbounds` 保留，但只剩读方**：那是编辑器模型之前被接管的节点（0.1.1 及更早）的存量数据，安装或改端口重新生成配置时必须继续把它们带上，否则那些 VLESS/SS/Socks 会在一次安装后消失。
> - **代价**：① 脚本给 anytls 换了端口之后，本节点的"自己的入站"仍留在老端口上（新监听照常进订阅，只是带后缀）——认定稳定性优先；② 面板不能再把任意监听"变成自己的"（用全局密码 + 节点原名），要这个效果就在表格里改它的凭据（会重启 sing-box 一次）；③ 端口段之外的监听（443 等）不会成为本节点入口；④ **「安装 / 更新」按钮仍是"整份重写"**，本修订没有改变它：在一台有本机 config.json、而面板从未写过的探针上点安装，仍会用模板（+`extra_inbounds`）覆盖那份文件，脚本手写的 `route`/`dns`/`outbounds` 会消失，面板入站那个监听也会改成全局密码。要不要让安装在"存在本机文件"时改为合并或拒绝，是一条独立决策，尚未做。
