...
- **默认模型是 `(provider, model)` 对**（不是单个模型 id）：同一模型可挂多个 provider，只存 id 无法定位走哪个网关。存 `settings`（`ai.default_provider_id` / `ai.default_model_id`）；发送框按 provider 分组下拉选择，另有"每节点会话临时覆盖"（不跨刷新保存）。**（实现修订 2026-09-18e）默认三件套再加一维：`ai.default_reasoning`（默认思考档位）**——面板档位留「默认(不指定)」的回合由服务端补上这一档；它与 (provider, model) 对**相互独立**（手选别的对同样生效），也**独立清空**（「清除默认」一次清三样）。写入只走 `PUT /api/ai/defaults` 一条路（`PUT /api/settings` 拒收该键）；存储档位若不被当次解析出的模型支持则**忽略回退未指定**，而不是 400 打断整轮——默认档位是对着默认对配的，面板可以手选任何对。
