...
| `settings` | key, value, encrypted | 全局 anytls 密码（服务端自动生成，§10.1）、**AI 执行策略与默认模型**（`ai.default_policy` / `ai.default_provider_id` / `ai.default_model_id` / `ai.default_reasoning`（2026-09-18e，写入只走 `/api/ai/defaults`，不走 `PUT /api/settings`），§12.1；思考预算为内置常量、不在此表；原 `ai.base_url` / `ai.model` / `ai.api_key` 三键 2026-09-18 起摘除、存量密文留库不读）、Telegram、保留期、延迟测量频率（`latency.interval_seconds`，默认 5）等 |
...
| `node_network` | node_id, iface, mode(in/out/both/max), quota_bytes, cycle_type(none/month/quarter/year), next_reset_at | 流量口径、配额与独立流量周期；空 `iface` 表示 agent 自动选择默认路由 |
| `node_billing` | node_id, cycle_type(none/month/quarter/day/year), cycle_days(单位随 cycle_type), next_due_at, note | 缴费周期 |
...
| `subscription_entries` | subscription_id, node_id, relay_node_id, proto, src_port, iface, alias, enabled | §10.2 的订阅绑定单位 = 入口；`relay_node_id=''` 为直连（**2026-09-18 起 `src_port` = 该入站的监听端口，一条入站一行**；`src_port=0` 是旧节点级行，由 reconcile 拆分），`enabled=0` 是**墓碑**（取消勾选，reconcile 不再加回） |
...
| `quick_commands` | id, name, command, sort_order, created_at, updated_at | **2026-09-19 新增**（§11）：终端页右栏「常用命令」的命令片段；明文存储（与 `ai_messages` 里留档的命令全文同待遇），服务端从不执行它 |
...
- **流量周期独立于缴费周期**（2026-09-16 修订；2026-09-18 增按季）：每节点设置 `none|month|quarter|year` 与「下次重置时间」（Unix 秒，表单精确至秒）。`none` 不隐式回退到自然月，按累计流量计算；月/季/年模式将已到期的下次重置时间依探针时区向前滚动，保留设置时的日、时、分、秒（季 = 3 个月步长，与按月共用同一套月末/闰日钳制）。月末和 2 月 29 日落到不存在日期时钳制到该月最后一天，下一次仍以原始日期规则计算。缴费周期仅用于到期提醒，与流量统计互不驱动。
...
> - **`config.json` 是 fobe 独占的**：同一台机器上又跑 one-sing.sh 又由 fobe 托管，两边会互相覆盖同一份 `config.json`（one-sing.sh 加的协议会消失）。**2026-09-17 起有了正解**：先在面板接管（见本节末的修订），脚本那些入站成为期望状态的一部分，此后由面板统一管理；接管之后仍继续用脚本往里追加，是唯一还会被覆盖的用法。**2026-09-17b/c 两次修订把这条改写掉了**：文件本身就是真相，面板只读只编辑，脚本追加的入站下一轮上报就出现在面板与订阅里，没有"接管"这个动作，也不存在"整份重写"。**（2026-09-18c 澄清：这句只描述面板的自动收敛——显式「安装 / 更新」至今仍是整份重写，只是不再改变监听端口与凭据，见本节末该修订。）**
...
> - **名字去后缀**：17g 给每条出站名追加 `协议:端口`，实战读起来就是"订阅名称字段不生效"。后缀删除，客户端看到的就是回退链算出的基础名（入口 alias → `nodes.sub_name` → `nodes.name` → 节点 id）；直连入口的 alias 现在真正逐字生效（此前 `NameSuffix` 未清，alias 后面仍会被追加一次后缀，后续入站甚至双重后缀）。同一节点多条入站同名时交给渲染器既有的兜底去重（Clash ` #2`、sing-box tag `-2`）——**代价（说清楚）**：`-2` 不含端口信息，光看名字分不清是哪条入站；要区分就用 alias 或别在一台机器上开同名入站。这与 2026-09-16 的 tag 修订是同一款取舍："拒绝静默加后缀"的口径从来只约束显式 alias（`alias_conflict`），自动名一直允许 `-2`。**（2026-09-18：这条代价收窄了——直连入口按入站拆行后，同一节点多条入站的自动名带上 `:<端口>`，只有真正同名的 alias 才落到 `-2`；"单条入站不加后缀"的口径保持不变，见 §10.2 该修订。）**
...
> **实现修订 2026-09-18b（搬移端口要带上订阅绑定）**：上一条把改端口做成一次搬移之后，订阅侧的绑定也必须跟着走——§10.2 实现修订 2026-09-18 起**一个直连入口 = 一条入站的端口**，绑定行里存的是端口；搬移只改文档与状态行的话，所有绑定过这个入站的订阅都会指向一个探针不再服务的端口，客户端在下一次拉取时**静默丢掉这个节点**（旧版"入口 = 节点"时它们会自己跟上，那是节点级身份的意外好处）。修法：搬移的两半之外再追加一对意图——`apply` 把该节点在所有订阅里 `src_port=from` 的直连行改写成 `to`（alias/enabled 一并带过去，墓碑照搬），`undo` 反向改回；推送成功且确有行被改写时记一条审计 `subscription_entry_port_move`（`from->to (N entries)`）并广播 `subscriptions_changed`。目标端口**已存在**行时以既有行为准（不覆盖操作员自己起的名），把源行删掉而不是留着悬空。**代价**：搬移窗口内（探针应用并重报前，通常数秒）新端口还没进上报文件，该行在订阅里渲染不出内容、在入口列表上标 `inbound_gone`——这是诚实的"探针还没应用"，窗口结束后自己收敛；而在**面板之外**改端口（手改 `config.json`、`jq` 重写）面板无从推断映射，绑定行同样会停在旧端口上标 `inbound_gone`，在入口列表里重新勾选新端口即可（面板不猜，见 §10.2 该修订的代价）。

> **实现修订 2026-09-18c（安装/更新按探针上报的文件取端口：不再把入站搬回旧值或换成随机端口）**：17b 把"文件是真相"用在**编辑**上，但**显式安装/更新**（节点页的「安装 / 更新」按钮，`POST /api/nodes/{id}/singbox/install`）至今仍是整份重写，而它取的端口一直是 `node_singbox.port`（记账列）。复现出两条：
> - **面板装过、编辑器搬过端口**：操作员把监听从 22039 改到 22040（探针已应用并重报）之后，再点一次「安装 / 更新」，监听被搬回 **22039**——因为编辑器搬移**不会**更新记账列（`pushConfigDocument` 只更新 `config_hash`，探针上报的 `Port` 又是期望帧的回声，两处永远同值）；更糟的是凭据按端口查，22039 下已经没有凭据，install 于是**静默 mint 一份新密码**：端口与凭据同时变，所有客户端立刻失效。
> - **纯 one-sing.sh 的发现型节点**：17g 起该列恒 0，install 于是走 `RandomPort()`，落到一个 10000–60000 的随机端口，脚本入站一并消失（"服务器上的 sing-box 入站被重设成另一个端口"就是这两条）。
>
> 修法（`inboundPortFor` + `nodeProxyPassword` 按序多端口查找）：
> - **端口解析顺序**：显式请求的 `port` → **探针最近上报的文件里那条 anytls 的 `listen_port`**（`store.SoleAnytlsPortInDoc`）→ `node_singbox.port` → 都没有才随机。文件优先于记账列正是"文件是真相"该有的样子；记账列退成"探针从没上报过文件（旧 agent）、或文件里有多条 anytls（无从选择）"的兜底。装完把解析结果写回 `node_singbox.port`，列与文档从此一致。
> - **凭据按同一顺序查**：先在**正在重建的那个端口**上找（文件里就有），找不到再退回原来的记账端口（API 显式改端口那条老路径：凭据还留在旧端口下），两者都没有才 mint。只按新端口查就是上面那次静默轮换。
> - **启动模板同步（`SyncSingboxConfigs`）用同一套解析**：它同样重建整份文档，此前也读记账列——手工在探针上改过端口、又没有编辑器标记的节点，会在模板变化时被搬回去。现在按上报文件的端口重建，并把该端口写回记账列。
> - **删除 `Store.SolePanelPort`**：无人调用，且用 nil Cryptor 去读加密快照，永远读不出内容——留着只会诱使下一个人踩同一个坑（HTTP 层改用 `SoleAnytlsPortInDoc` + 进程 Cryptor）。
> - **口径澄清**：17b 的"不存在整份重写"只针对**面板的自动收敛**（启动同步跳过被面板编辑过的节点）。**显式「安装 / 更新」至今仍是整份重写**——它会丢掉脚本手写的 route/dns/outbounds 与编辑器加的非模板入站。本次只保证它**不再改变监听端口与其凭据**，没有改变"整份重写"这件事本身。**（2026-09-19 推翻后半句：安装/更新对配置也改走读-合并-写，见下条修订。）**
> - **代价**：① 文件里那条 anytls 成为面板入站的落点，操作员手改文件把它挪走时 install 会**跟随**文件而不是把记账列写回去（这正是本次要的语义，但意味着记账列不再是权威）；② 文件里有多条 anytls 时仍回退到记账列/随机——要固定端口先把它改成唯一那条 anytls 再安装。

> **实现修订 2026-09-19（安装/更新对配置也走读-合并-写：不再整份重写）**：18c 的口径澄清把「显式安装/更新仍是整份重写」留成已知代价，实战反馈是它站不住：面板装过、又在编辑器里加过 VLESS/Socks 的节点，点一次「安装 / 更新」就只剩 anytls——"更新个版本"的预期里不包括"我的入站没了"。本修订把 17b 的"文件是真相"推进到显式安装上（`POST /api/nodes/{id}/singbox/install`，`applySingboxDesired`）：
> - **有上报文件就以它为基底**：解析最近上报的 config.json，它声明的一切原样保留（其他协议入站、route/dns/outbounds、面板入站上被手改过的字段）；安装只保证一件事——**面板自己的 anytls 入站在解析出的端口上**（端口解析顺序照旧 18c）。文件在该端口已有 anytls 就**原样保留**（凭据是刚读回来的，重写证书路径或 padding 反而是覆盖手改）；没有才插入模板入站，此时才允许 mint 凭据（"创建入站"语义，§10.1）——插入的监听照编辑器惯例先在 `node_singbox_inbounds` 落一行 `pending`（推送失败回滚），探针应用重报后由 reconcile 接管。
> - **无上报文件才落模板**：`BuildNodeConfigWithInbounds` + 存量 `extra_inbounds`（0.1.1 接管节点的老路）不变。上报文件解析失败（坏 JSON）也走模板——安装本就是坏配置的修复路径，不修反而是把故障留住。
> - **`singbox_edited` 标记改为条件设置**：合并产物与"模板会生成的字节"逐内容比较（两边都过一遍 JSON 解码-再编码，键序与缩进不参与），等价 ⇒ 不标记、按模板字节存储（内容等价的键序差异不值得一次下发抖动，启动模板同步照旧管理它，模板修复仍能到达）；不等价（文件里有操作员自己的内容）⇒ 标记，模板同步从此不碰这个文件。
> - **端口冲突拒绝而不是静默覆盖**：解析出的端口被文件里**非 anytls** 的入站占着时，安装回 `400 duplicate_port`——旧行为会把那个入站从生成物里悄悄丢掉，连闸门②都不用拦。
> - **代价（说清楚）**：① **模板演进只到达"文件与模板等价"的节点**——编辑器写过、或安装时文件里已有操作员内容的节点拥有自己的文件，模板修复（如 DNS 段改版那种）要靠编辑器或卸载重装到达；② 非特权探针上报的文件带重定位前缀，逐内容比较必然不等——它们在第一次安装/更新后就不再吃模板同步（接受，换判据简单）；③ 安装按**最近上报**的文件合并，探针离线期间对文件的手改会被合并结果覆盖（最后写赢，17b 代价②在安装上的延伸，安装没有 `reported_hash` 可校验）；④ 面板自己的 anytls 入站从此不被安装"重新扶正"——证书路径、padding 被手改坏的要靠编辑器修；⑤ 随机端口与文件里现有监听撞车时同样回 `duplicate_port`（重试即可，概率 ~1/50000）。
...
- **订阅绑定的单位从「节点」变成「入口」**：身份 `(node_id, relay_node_id, proto, src_port, iface)`，`relay_node_id=''` 即直连——**2026-09-18 起，一个直连入口 = 该节点的一条入站**（`src_port` 就是那条入站的监听端口，`proto/iface` 留空；见下方该修订）。渲染时一个入口 = 一个 anytls outbound：

| 入口 | server | port | 证书 pin |
|---|---|---|---|
| A 直连（每条入站一行） | `A.primary_ip` | 该入站的 `listen_port` | A 的自签证书 |
| B 直连（每条入站一行） | `B.primary_ip` | 该入站的 `listen_port` | B 的自签证书 |
...
- **落账 + 墓碑（自动纳入，2026-09-16c 修订）**：新表 `subscription_entries`（SchemaVersion 9→10）。当某订阅里**已启用**某节点 B 的直连入口时，B 的所有**可渲染**中转候选自动落成 `enabled=1` 的行并落审计 `subscription_entry_auto_added`；操作员取消勾选写 `enabled=0` 的**墓碑**而不是删行——reconcile 不会把它加回来。`sub.relay_auto_include`（布尔式设置，默认 on）关掉就退化成「只作候选、手动勾选」。触发点：服务端启动、探针上报新的 forwards 快照（hub 观察者）、面板读候选列表。
  ...
  - **候选消失不删行**：转发规则被删、探针离线导致快照失真时，入口标 `unavailable` 并在渲染时跳过，但**仍留在面板上可取消**——不静默消失，否则客户端的节点集合会毫无提示地变。此时 B 直连入口还在，客户端的降级是合理的。**（2026-09-19b 修订：这条只在不删会留下"永远渲染不出来的行"之外仍然成立——规则真的没了、或落地节点真的没有 anytls 入站时，行会被删除；"快照失真/读不到文件"这类**没有证据**的状态仍然照此保留，见下。）**
- **候选范围（§10 的限制继续生效，且按入口逐条判定；2026-09-17i 修订：判定与订阅渲染同源，读上报的文件而不是管理态列——发现型（one-sing.sh）节点的直连与中转入口照常进选择器，落点端口 = 上报配置里的 anytls 入站集合；2026-09-18 修订：直连候选按**每条入站的端口**逐条列出，一条入站一行）**：选择器只**列出会出现在输出里的入口**。直连入口要求该节点可渲染（主 IP + 上报文件可产出出站，或管理态端口+证书），且**该入站本身**在可渲染集合里；中转入口还额外要求**落地节点**有带证书的 anytls 入站——中转是"借道"，最终 TLS 会话仍然终止在 B，所以 B 必须是可用的 anytls 节点；**跳板 A 自己不需要装 sing-box**（它只转发包），但必须有主 IP 才谈得上入口。已绑定但**当前**不可渲染的入口仍然列出（灰显 + 原因、可取消勾选），否则操作员保存一次就会把绑定关系悄悄丢掉——这与 §10 原口径完全一致。**唯一例外是两类"再也渲染不出来"的行：`inbound_gone` 的直连行、腿已经没了的**中转行（规则没了，或落地节点已无 anytls 入站）——reconcile 直接删除它们（2026-09-19b 修订，见下），**墓碑一并删**；判定缺证据时（探针读不了规则、落地节点没有上报文件）照旧留在面板上可取消。
- **命名（每个节点可自定义）**：
  - 直连入口：`alias` → `nodes.sub_name`（新增列，面板「编辑服务器 → 订阅名称」）→ `nodes.name` → 节点 id；**该节点有两条以上可渲染入站时，自动名追加 `:<端口>`**（2026-09-18 修订，见下）。
  - 中转入口：`alias` → `sub.relay_name_format`（默认 `{name} · {relay}:{port}`；占位符 `{name}{relay}{region}{flag}{relay_region}{relay_flag}{host}{port}{proto}{iface}`，必须含 `{name}`）。`{name}` = 目标节点的最终基础名，`{relay}` = 跳板节点名。**2026-09-19c 修订：新增 `{region}` / `{relay_region}` 与 `{flag}` / `{relay_flag}`**——落地/跳板节点的 §14 国家码（geoip 或手动钉的值，HK/CN/JP 风格）及对应国旗 emoji（两个区域指示符，与前端 `flagEmoji` 同一编码）；节点没解析出国家时 region 展开为 `XX`（即手动国旗已接受的"未知"码）、flag 展开为 🇽🇽，这样占位符两侧的分隔符不会被吃掉，名字形状保持稳定。默认带 relay + port 是必需的：多条规则指向同一 B、多个中转指向同一 B 时默认名不能撞车。
  - 出站 tag 仍由最终名派生 `fobe-<name>`（重名 `-2` / `#2` 留作兜底），但**显式 alias 撞名在保存时就拒**（`alias_conflict`）：静默加后缀会毁掉写模板的人的预期。**判定口径（2026-09-19 修订）**：别名按**字符串**比，与入口属于哪个节点、哪条中转无关（同一节点的两条入站、或同一目标经两条不同中转，都是同样的两条入口）；只比这次保存里**被勾选（会实际渲染）**的入口——墓碑渲染不出东西，它上面留着的旧名不参与；同一份 payload 里重复出现的同一入口身份只算一条，不计两次撞名。旧节点级行拆分时会照抄 alias，于是两条入站行可能同名：保存被拒**一次**、面板把两个字段就地标红，改掉其中一个即可——比让客户端悄悄多出一个 `-2` 后缀更可预期。alias 1–64 字符、禁控制字符；两个渲染器都走 escape，别名只出现在带引号的标量里，没有注入面。
  - **代价**：改 alias/名字 = 改 tag，模板里 pin 过 `fobe-<旧名>` 的 selector/urltest/final 会失配——重拉订阅即恢复（订阅本来就是干这个的，同 §10 实现修订 2026-09-16 对 tag 的取舍）。
- **接口**：`GET /api/subscriptions/{id}/entries`（候选 + 已绑定，含 `selected/alias/auto_name/available/reason/warning/discovered`；**2026-09-19 起每行带 `protocols`**——该入口渲染成的线上协议（anytls/vless/…），取自探针上报的 config.json（正在运行的配置是唯一准绳，不读管理态列）：直连行 = 该端口入站的 type（旧节点级行去重列出全部），中转行恒为 `anytls`（中转腿只能终结在 anytls 入站上），文件未上报时由管理态端口+证书对兜底为 `anytls`；该行标 `inbound_gone` 时 `protocols` 为空——运行中的配置已经不给答案了，面板不猜）；`PUT /api/subscriptions/{id}/nodes` 接受 `entries`（**`node_ids` 保留为兼容路径**：等价于只维护直连入口，中转行不动）；`PATCH /api/nodes/{id}` 接受 `sub_name`。§17 快照带上入口与 alias（老快照只带 `node_machine_ids` → 导入为直连入口，与旧行为等价）。
...
> **实现修订 2026-09-18（直连入口拆到「每条入站一行」：入口列表分开显示，重名靠端口区分）**：本节的绑定单位一直是「入口」，但直连入口实际上还是**节点级**的——身份 `(node_id,'','',0,'')`，一个节点只有一行。于是一台跑了两条 anytls 入站的服务器在入口列表里只占**一行**（勾一次就把两条都发出去），两个出站只是同名，客户端里只能靠渲染器的 `-2` / `#2` 兜底区分，光看名字分不出是哪条入站（这正是 17j 删掉 `协议:端口` 后缀时写明的代价）。现在把这一层补齐：
> - **直连入口 = 一条入站**：身份 `(node_id, '', '', <该入站的 listen_port>, '')`。`src_port` 的语义统一成「客户端拨的端口」（直连＝该节点自己的入站端口，中转＝跳板转发规则的源端口），`proto/iface` 对直连一律留空——直连没有 nftables 规则可对齐。**表结构不用动**：`src_port` 本来就在主键里（§6），只是此前直连行恒为 0。
> - **候选与渲染同源**：候选端口来自渲染器自己的输出（`nodeIngressPorts` = 上报文件能产出的出站端口；文件还没上报时用管理态端口+证书兜底），所以"选择器列出的"与"客户端拿到的"逐条对齐；每条入站独立判定可用性，某条入站从文件里消失时该行标 `inbound_gone`（灰显、仍可取消），而不是让整个节点连不通。遮蔽告警（§21.9）同样按端口逐条判。
> - **旧行自动拆分**：升级上来的行是节点级（`src_port=0`），reconcile 在**知道该节点入站之后**把它拆成每端口一行（alias 与 enabled 一并带过去，墓碑照拆），并写审计 `subscription_entry_split`——不拆的话选择器会同时列出旧行与新候选，勾一个新端口就会把节点渲染两遍。拆分挂在与中转落账同一批触发点（启动、探针上报、打开选择器），且**不受 `sub.relay_auto_include` 开关约束**（那是中转的开关）；拿不到入站的节点保留旧行，旧行照旧渲染该节点全部入站，下一个触发点再试。
> - **重名只在需要时加端口**：节点有 **≥2 条可渲染入站**时，直连自动名 = 基础名 + `:` + 端口（`东京:8443`）；只有一条时**保持原名不加后缀**（17j 的结论不变：绝大多数节点只有一条入站，`nodes.sub_name` 必须逐字生效）。显式 alias 依旧逐字优先（拆分照抄 alias，两条入站行因此可能同名：保存会被拒一次并在字段上标出，改掉其中一个即可——口径见本节「命名」条）。**代价（说清楚）**：多条入站的节点，客户端看到的 tag/name 会从 `B`/`B-2` 变成 `B:8443`/`B:8444`，pin 过旧 tag 的模板要重拉订阅（同 §10 实现修订 2026-09-16 对 tag 的取舍）。
> - **代价**：① 端口成为绑定身份的一部分，所以**面板之外**改端口（手改 `config.json`、`jq` 重写）会让绑定行停在旧端口上、标 `inbound_gone`——旧版节点级入口在这种情况下会"自己跟上"，现在面板不猜映射，重新勾选新端口即可；**面板内**改端口不受影响（§9.3 实现修订 2026-09-18b 让绑定随搬移一起走）。② 删除一条入站不再静默降级成"该节点剩下的入站"：那条绑定行会标 `inbound_gone` 等操作员处理——语义上更准（客户端配置里的那条 outbound 确实没了），代价是多一次点击。③ 探测型节点的订阅仍滞后一个上报周期（不变）。

> **实现修订 2026-09-19（入口列表一行一个身份；alias 撞名的判定口径）**：两条 nftables 规则可以共用同一个 `(proto, src_port, iface)`（§21 用 handle 把可编辑的规则分开），但它们描述的是**同一个入口**。候选装配原先按规则逐条出，于是选择器把同一条入口列成两行——前端按身份给行做 key（改一行只该改那一行），两行同 key 时 `patch` 会把名字同时写进两行，最后输入的那个盖住两行，保存回来就是 `alias_conflict`，而操作员看到的是「我给同一节点的两个入口起了不同的名字却存不进去」。修法三道：`relayCandidates` 按身份去重（同名时保留第一条非空规则备注）、`subscriptionEntryViews` 再拿 `seen` 兜一道（身份被列过就不再列）、前端 `dedupeEntries` 作为第三道防线并在字段上就地标红重名。同一次把 alias 撞名的口径写清楚（见本节「命名」条）：按字符串比、与节点/中转无关，只比勾选的入口，同一 payload 里的重复身份只算一条；面板对撞名字段就地标红。**入口身份里带 `relay_node_id`**，所以**同一目标经两条不同中转是两个不同的入口**（各自可改名，绝不被合并）——被合并的只有 `(node, relay, proto, src_port, iface)` 完全相同的重复候选。**代价**：两条共用 tuple 的规则在选择器里只剩一行，区分它们要去 §21 的转发表格（那里按 handle 列）。

> **实现修订 2026-09-19b（失效的入口行由 reconcile 清理，不再留成永久灰行）**：上一条修订把绑定身份从节点改成**端口**，代价之一是**面板之外**改配置（手改 `config.json`、`one-sing.sh` 重写）会让绑定行停在一个探针不再服务的端口上、永远标 `inbound_gone`。那行渲染不出东西、客户端看不到，却只能取消勾选变成墓碑——而行本身照样列在那里（墓碑也列出），实战反馈就是「订阅里依旧有这行，怎么删都删不掉」。现在 reconcile 在拆分与中转落账**之前**先清理两类「再也渲染不出来」的行，记审计 `subscription_entry_pruned`、广播 `subscriptions_changed`：
> - **直连行**（绑定与墓碑一并）：端口不在 `nodeIngressPorts` 里就整行删除。直连行没有自动纳入路径（没有任何代码会自己写直连行），墓碑拦不住任何东西，所以跟绑定行一起走；端口日后回来时，选择器把它当**全新未勾选候选**。
> - **中转行（墓碑一并）**：中转行是从两半推导出来的——跳板 A 上的一条 tcp 转发规则，目标落在落地节点 B 的可 pin anytls 入站上。**任一半没了，这条腿就没了**，行也渲染不出来（要么转发指向一个没人监听的端口，要么 TLS 没有终结处），于是整行删除：规则不再落在这台落地节点上、或 B 的上报文件里已经没有可 pin 的 anytls 监听。**墓碑也删**——判据是"这条腿还能不能自己回来"：只要两半都在（规则 + 落地 anytls），reconciler 下一次 reconcile 就能把它落账回来，那时墓碑是操作员的**有效**选择，必须留着（删了等于把入口带着勾还回去）；而两半缺一时它回不来，墓碑也拦不住任何东西，留着就是一行永远渲染不出来的灰行。日后拓扑恢复（规则重新指向、或落地节点重开 anytls），reconciler 会**重新落账为启用行**——这是 §10.2 的默认行为，不是意外（2026-09-19 用户确认）。
>
> 四道闸门与边界：
> - **必须有上报文件且能产出出站**（`rd.live` 非空）：文件缺失/解析失败/只有管理态端口证书时，面板读到的是 `not_ready` 而不是 `inbound_gone`——那种状态下"没了"与"探针还没应用"无法区分，一行都不动。
> - **判定与选择器、落账同源**：直连用 `nodeIngressPorts`、中转用 `relayCandidates` 的产出（身份集合）与 `relayableInbound`——都是渲染器/落账自己用的那几个函数。**注意不是** `targetReadiness.relayable`（它折进了管理态端口/证书兜底，而有文件时那份兜底不是真相）。"规则侧"的判据还要过一层**快照可信度**：只有 `node_forward_status.supported=1`（探针确实能读 nftables）时，空规则列表才等于"被删了"；读不了（nft 缺失/无 root/§21 之前的 agent）是"未知"，不动。同理，跳板节点已删、或落地节点没有上报文件，都算没有证据。
> - **直连行跨不过 `pending`/`deleting` 生命周期行**：那是面板自己刚加、刚搬的端口还没被探针应用（§9.3 实现修订 2026-09-18b 让绑定随搬移一起走）——缺这道闸门，**每次搬端口都会把刚搬过去的绑定删掉**（搬移窗口内新端口读 `inbound_gone` 是预期，探针一上报就收敛）。中转行不需要这道闸门：面板搬/删监听时旧端口会一直留在文件里直到探针应用，整段窗口内这条腿仍然可推导。
> - **面板外改端口也算"规则侧没了"**：规则还指着 B 已经被搬走的旧端口时，客户端根本连不通（DNAT 打到一个没人监听的端口），选择器本来就标 `relay_gone`——行会随清理消失，把规则重新指到新端口后自动重新落账。
>
> **代价（说清楚）**：① 行删了 alias 与（直连的）墓碑就没了，端口/入站/规则日后重新出现时是一个**全新未勾选候选**（中转行则会按默认行为自动落账为启用行），得重新勾选、重新起名；② 面板外"删掉又很快加回来"、中间正好跑过一次 reconcile（打开选择器/重启/转发上报）时会丢绑定——但那个窗口里该行本来就不渲染，客户端无感，面板不猜映射；③ **中转墓碑不再是持久挡箭牌**：它只在两半都在时有效，拓扑一断就被清掉——这是 2026-09-19 明确选定的口径（宁可列表干净，接受拓扑恢复时重新落账）。
...
- **终端页是全宽工作区 + 可拖拽分栏（实现修订 2026-09-18e）**：该路由退出 `.content` 的 1440px 阅读栏上限（`content-wide`），只留 24px 页边距——两栏并排时每一像素都互相挤压，阅读栏的排版约束在这里只损失可用宽度。终端 / 助手两栏之间有一条 10px 拖拽手柄（pointer capture，双击恢复默认），比例存 localStorage（`fobe.terminal.ratio`，0.25–0.75 夹紧——再窄助手放不下发送行、再宽终端成了细条）。比例以两个 CSS 变量（`--tw-col` / `--tw-rest`，**直出 `Nfr` 字面 token**）下发：不用内联 `grid-template-columns`（会压过 ≤900px 的单列媒体查询），也**不用 `calc(N * 1fr)`**——fr 进 calc 在不支持的浏览器里整条 `grid-template-columns` 声明被丢弃，grid 退化成单隐式列，助手掉到终端下面（2026-09-18e 真机踩过，`var()` 纯替换是唯一两全的做法）。
...
- **终端 shell 的解析（2026-09-20 修订）**：agent 起 PTY 的 shell 按 `$SHELL` → **`/etc/passwd` 当前 uid 的登录 shell** → `/bin/sh` 解析（`localShell()`，子进程环境缺 `$SHELL` 时由 agent 补上）。此前中间缺 passwd 一步，而 systemd/procd 的服务环境里没有 `$SHELL`，于是 web 终端在 Debian/Ubuntu 上跑的是 dash、在 OpenWrt 上跑的是 busybox ash——**机器上装着新版 bash 也没用**（SSH 是 sshd 按 passwd 拉起登录 shell，web 终端此前不是）。dash/ash 没有行编辑器、开不了 bracketed paste，右栏多行常用命令在这类机器上一律禁用；换成登录 shell 后，"多行是否可点"如实反映该机器 SSH 所见。passwd 里 nologin/false 的账号按"无交互 shell"处理（与 sshd 同口径），仍回 `/bin/sh`；Alpine 官方镜像的 root shell 本就是 `/bin/sh`，行为不变。前端配套：多行按钮的禁用态不能在渲染时读 `handle.bracketedPaste()`——bash 是**画出第一帧提示符之后**才发 `?2004h`，此后没有任何渲染触发点，按钮会永远停在禁用快照上；Terminal 现在用 `parser.registerCsiHandler` 盯 `?2004 h/l`，把模式翻转实时推给页面状态。
...
- **右栏改为标签容器 + 「常用命令」标签（2026-09-19 新增）**：AI 助手面板不再是右栏的全部——右栏变成一个标签容器，v1 两个标签：「AI 助手」「常用命令」。可用性门控（只渲染可用的标签，用户拍板）：AI 已配置（`ai_configured`）**或**存在常用命令时右栏显示；两者皆无时右栏隐藏（终端独占，原 solo 模式）；只配了一侧时**不出标签条、直接单面板**，两侧都在时默认选中 AI 助手。实现上有三条硬规则：
  - **两个面板常驻挂载**（`hidden` 属性切换可见性）：AssistantPanel 的 transcript、流式 SSE 与会话 id 全在组件自身状态里，切标签即卸载会掐断进行中的回合、清空历史——所以只许隐藏、不许卸载。连带 `.ai-panel` 的定高（`min(720px, calc(100vh-180px))`）上移到 `.terminal-side` 外壳：外壳是一张卡片、内部面板 flex 拉伸，否则会出现"卡中卡"。
  - **点击的执行语义**：命令经 `TerminalHandle.sendInput()` 以 `terminal_input` 帧注入自己的 PTY（= 操作员手敲，PTY 自己回显，服务端不经手执行）。**单行**命令 = 命令文本 + `\r`，点击即输入并执行（不加防误触，用户拍板：与手敲等价）；**多行**命令 = bracketed paste 序列（`\x1b[200~ … \x1b[201~`）**不带结尾回车**——整块粘到命令行上、可检查、回车才执行。回退判定读 xterm 实例的 `modes.bracketedPasteMode`（点击时实时读：shell 在提示符出现后才开启该模式）：shell 没开（busybox ash / OpenWrt 从不开）时**禁用多行按钮并提示**，而不是原样发送——对不支持安全粘贴的 shell，唯一能做的"粘贴"就是逐行执行换行，那违背"多行仅粘贴"的全部意义。
  - **断连即禁用**：Terminal 组件新增 `onConnectionChange` 把连接状态提到页面层，断连时命令按钮禁用（此前 `send()` 在 WS 非 OPEN 时是静默 no-op，点了没反应才是最差的反馈）。点击后焦点落回终端（多行粘贴后必须立刻能按回车）。
  - **数据与接口**：`quick_commands` 表（id/name/command/sort_order，SchemaVersion 15→16，`schema.sql` 的 `IF NOT EXISTS` 天然覆盖老库）；接口 `GET/POST /api/quick-commands`、`PUT/DELETE /api/quick-commands/{id}`、`PUT /api/quick-commands/order`（整体重排），全部会话鉴权。校验在服务端：名称 ≤100 字符；命令 ≤16 KiB 且**单行 ≤4000 字节**（Linux 行规程 MAX_CANON=4095，超长单行发进 PTY 会被截断——这不是显示问题，是"截断后的命令照跑"的事故）；CRLF 归一为 LF、去掉尾部换行（Windows 剪贴板的 `\r` 不能烤进 PTY 输入）。上限 50 条。
  - **审计：不写**。这是操作员自己的键盘注入，服务端从不执行命令内容，与 §1054「命令执行入口收敛到 AI 确认流」的辨析：那条决策拆掉的是**服务端下发 shell 的入口**（命令队列 `POST /api/nodes/{id}/commands`），而常用命令走的是操作员自己的交互终端——服务端只转发键盘帧、看不见执行结果，语义与手敲完全等价，不构成新的下发面。**它也不进 AI 工具集**（§12.7 的三个终端工具不动）。
...
- **默认模型是 `(provider, model)` 对**（不是单个模型 id）：同一模型可挂多个 provider，只存 id 无法定位走哪个网关。存 `settings`（`ai.default_provider_id` / `ai.default_model_id`）；发送框按 provider 分组下拉选择，另有"每节点会话临时覆盖"（不跨刷新保存）。**（实现修订 2026-09-18e）默认三件套再加一维：`ai.default_reasoning`（默认思考档位）**——面板档位留「默认(不指定)」的回合由服务端补上这一档；它与 (provider, model) 对**相互独立**（手选别的对同样生效），也**独立清空**（「清除默认」一次清三样）。写入只走 `PUT /api/ai/defaults` 一条路（`PUT /api/settings` 拒收该键）；存储档位若不被当次解析出的模型支持则**忽略回退未指定**，而不是 400 打断整轮——默认档位是对着默认对配的，面板可以手选任何对。**（实现修订 2026-09-18g）面板不再以「默认(不指定)」开局**：终端页 AI 侧栏的思考档位下拉在挂载与换模型时都**预选 `ai.default_reasoning`**（仅当所选模型确实暴露这一档，`web/src/pages/TerminalPage.tsx` 的 `defaultReasoningFor`），模型不提供该档才回落到「默认(不指定)」——操作员看到的「默认(不指定)」与设置页写着的 `high` 打架，读起来像默认档位根本没被采用（本条就是用户实测报的现象）。服务端「未指定即补默认档位」的回退**一字未改**，仍是 `/api/ai/chat` 与 `/api/ai/chat/continue` 两条路的兜底（端到端由 `TestAIChatAppliesStoredDefaultReasoning` 盯住）；预选只让面板**发出的档位**与它**显示的档位**一致。代价：① 「默认(不指定)」从开局面变成显式选择项（仍可选手工未指定）；② 面板挂载后不再跟随设置页对默认档位的后续改动（与模型下拉同一条口径：要拿新值刷新页面）。**（同一条修订的另一半）设置页「默认思考档位」下拉的选项改为所选默认模型自己的档位集合**：此前它列的是全量词表 `off / minimal / low / medium / high`，于是能选到当前模型根本不支持的档位（选完只会被服务端按"不支持就回退未指定"静默忽略，读起来像设置没生效）。现在选项 = 该 (provider, model) 的 `effective_levels`（协议能力 ∩ 模型 `reasoning_options`，与面板同源）；**卡片显示与写回的档位都取同一个钳制值**（`web/src/pages/SettingsAI.tsx` 的 `effectiveReasoning`）——草稿里那档一旦不在当前对的集合里就既不下拉显示（React 会静默落到第一个 `<option>`，而 state 留着旧值仍会被保存）也不写回；模型不提供任何可调档位时按 §12.5 的家规**用文字说明，不渲染空下拉**。服务端的校验口径不变：`PUT /api/ai/defaults` 仍只校验统一词表（默认档位是跨对偏好，不是对的属性），"当次模型不支持就忽略"那条规则留给回合时生效。
...
- **什么都不含的助手轮既不上行也不落库**（2026-09-18c 实现修订）：上游首个 delta 之前就失败、或操作员立刻停止时，会留下一条 `content=''` 且没有 `tool_calls` 的 assistant 行；`openai-completions` 没有原生块可重建，只能编码成光秃秃的 `{"role":"assistant"}`，网关据此 400 掉该会话此后的**每一次**请求（`Invalid assistant message: content or tool_calls must be set`）。写入端（`persistAssistantTurn`）与编码端（丢弃该轮，**不是**改写成 `content:""`——空串在多数网关的校验里同样是"没设置"）都要挡，前者止血、后者救存量会话。详见 §12.7 实现修订第 6 条。
...
> 6. **什么都没产生的助手轮不得落库，也不得上行（2026-09-18c）**。上游在首个 delta 之前就失败（502 / 截断流）或操作员立刻按停止时，循环原本照样写一条 `content=''`、`blocks={}` 的 assistant 行——"半截轮也落库"的本意是**已经流给操作员看过的内容**不许回滚，空轮并没有这层内容，面板侧本来也会丢弃它（`historyToEntries` 跳过既无文本又无思考的助手行），所以这一行在界面上什么都不显示，**它的全部危害都在网线上**：`openai-completions` 没有原生块可重建（§12.5 表格「推理块回传＝无此概念」），它会编码成**光秃秃的 `{"role":"assistant"}`**，网关以 `invalid_request_error: Invalid assistant message: content or tool_calls must be set` 400 掉该会话**此后每一次请求**——故障出现在离病因很远的地方（操作员看到的是自己没写过的一条消息）。两层同时修：① `persistAssistantTurn` 在 text/calls/raw 三者皆空时不落库（有原生块的思考轮照旧落库、照旧回传）；② `openai-completions` 的编码器**丢弃**这类助手轮（不能改成 `content:""`——多数网关的校验是 JS 假值判断，空串同样落进"没设置"分支）。②是必须的：**存量会话里已经躺着这种行**，只在写入端修的话，被污染的历史永远打不开。
...
> - **实现修订（2026-09-18f：`Ctrl+C` 之后必须跟牺牲字节）**：清理键原本是 `\x15\x03`，紧跟着就发命令，而 bash 会把中断后**第一个被读到的字节**吞掉 ⇒ 命令首字符静默消失（`ip addr` → `p addr`）。真机复现与修法（`\x15\x03 `，末尾空格是牺牲字节）记在 §12.7.3 第 1 条；`send_keys` 侧同一机制不自动兜底，只写进工具描述，记在 §12.7.4。
...
1. **清理当前行**：先发 `Ctrl+U` `Ctrl+C` **再加一个空格**（`\x15\x03 `，常量 `aiTerminalClearKeys`），然后才发命令文本。**这是盲打**：前台若不是 shell 提示符（`vim` / `less` / `apt` 的对话框 / 密码提示），这几个字节是**发给那个程序的别的命令**，并且会杀掉操作员正在跑的任务、清掉他正在输入的命令。用户 2026-09-18 明确接受，并要求 **AI 在飞的时候操作员照常可输入**（后果由操作员承担，**不做锁、不做占用指示**）。
   - **2026-09-18f 实现修订（真机上"吞字符"）**：`\x15\x03` 后面**必须**跟那个空格，别把它当多余空白"清理"掉。bash 的 readline 在处理 `Ctrl+C` 引发的那次 SIGINT 时，会**丢掉当时已经就绪的那一个字节**——而紧挨着的正是命令的首字符。症状是最难查的那一类：屏幕（也是模型读回来的那一份）**完整回显**整行命令，真正执行的却少一个字（`ip addr` 执行成 `p addr` ⇒ `bash: p: command not found`、`$?`=127），于是模型看到的是"命令跑了、失败了"，而不是"我们的键把命令咬掉了一个字"。真机复现（Debian 12 · bash 5.2，同一串哨兵命令）：两帧紧挨着发 12 次里 10 次吞掉首字节（另一轮采样 9/12），把键与命令合成**一次**写入则 12/12 必吞，中间隔 2ms 则 12/12 不吞。macOS 的 bash 3.2 从不复现——所以这个洞一路活到了真机。牺牲字节选**空格**：行首空白不属于命令行本身，被吞掉没有任何副作用（唯一副作用是 `HISTCONTROL=ignorespace` 的主机上 AI 命令不再进 history，与 §12.4 记的"进 history"略有偏差，可忽略）；**不能用 Tab**——没被吞掉时它会在操作员的屏幕上触发 readline 补全。回归：`TestAIShellSentinelIsHowACommandFinishes` 里对"中断之后必须还有一个字节"的单独断言（常数相等断言挡不住有人把空格删掉）。
...
- **这一口 `send_keys` 也躲不掉（2026-09-18f 记，与 §12.7.3 同一机制）**：中断之后**下一个被读到的字节**（不看是否跨帧、跨调用）同样会被 bash 吞掉。这里**故意不自动补牺牲字节**——`send_keys` 的语义是"原样按键"，替模型改写按键会引入更危险的语义（它只想按一次中断，凭空多出的空格在 `less` 里就是翻页）。代价转嫁给提示词：工具描述已写明"Ctrl+C 放在载荷末尾，别在后面再跟按键"。
...
- **观测出口 IP（实现修订 2026-09-19）**：阿里云等云厂商的公网 IP 是网关侧 NAT，网卡上只有 VPC 私网地址——agent 只枚举网卡（`net.Interfaces`，§14 第一条），因此永远报不出公网地址；komari 之类的探针是靠 agent 请求第三方回显服务拿到的，fobe 不引这个依赖，改用**面板自己已经看到的那条路**：`/ws/agent` 升级时经 XFF 信任链（§4.3）解析连接源地址（nginx 后取 XFF 最左、直连取 socket 对端），当且仅当探针上报集**没有任何公网地址**时，把该地址以 `scope=public` 并入 `node_ips`，并作为主 IP 指认的候选——agent 在无公网可报时的自选按构造只能是内网地址，观测地址是这台机器唯一的公网身份。三条边界：①私网/CGNAT 源地址（纯内网部署、没配 XFF 的反代）不并入——不当公网冒充；②探针一旦自己报出公网地址即恢复"上报集全归 agent 所有"，观测行随本次全量上报自然退场；③手动主 IP 照旧优先于自动指认（指认前查 `manual_primary`，查询失败按"可能有钉"保守处理——漏一次自愈，覆盖手动钉不自愈）。副作用（收益）：GeoIP 国旗对纯 NAT 探针也能自动命中了——§14 修订 2026-09-16b 末句"纯内网又不带公网地址的探针只能靠手动钉"自此作废。重指认守卫相应放宽一条：存储的主 IP 仍在上报集内但**不是**当前候选、且无手动钉时也重指（旧守卫会保留被旧版写偏的值永不自愈，正是 §14 修订 2026-09-15 抱怨的那类漂移）。无 schema 变更，无 wire 变更（观测行只活在服务端库里）。
...
- 页面：登录 / 概览（卡片墙）/ 节点详情（**只读监控**：指标 + 图表 + 流量 + 延迟历史）/ 订阅与模板 / 延迟测量 / 告警 / 终端（全屏）/ AI 助手（侧栏）/ 设置（**通知**、AI、GeoIP、保留期、主密钥状态、**服务器**、**延迟测量**、**常用命令**、**审计日志**）。（2026-09-15 修订；2026-09-16 修订：详情页「命令」卡片移除，见下；2026-09-16 修订：通知从基础设置页独立为子页，见 §15.0；2026-09-16 修订：延迟目标页更名延迟测量并收编测量频率面板，见 §13；2026-09-19 修订：设置新增「常用命令」子页，终端页右栏改标签容器，见 §11）
- **配置入口收敛（2026-09-15；2026-09-16 列完善；2026-09-16 修订：添加节点按钮随之下沉到本页）**：设置里新增「服务器」页，表格列出已接入的服务器，明确显示文字状态、主 IP、过期时间、版本，行尾为编辑/删除图标按钮；未配置缴费周期时过期时间留空，已配置时显示距离 `next_due_at` 的剩余或逾期时间。**到期列改印整天数（实现修订 2026-09-21）**：该列不再用 `fmtDueDuration` 的「18d 3h」，改用 `fmtDueDays` 的四舍五入整天数——「18天」/「已逾期 18天」，小时分量彻底不出现；剩余整天数 < 7（含已逾期）整格转红（`.due-days.due-soon`，明暗主题各取 `--red` 一档）。两处取舍：①**阈值与显示取同一个取整值**，因此不会出现「7天」却是红的（剩 6.5 天记 7 天、不红；剩 6.4 天记 6 天、红）；②**逾期方向向上取整且保底 1 天**，否则逾期三小时会印成「已逾期 0天」，读起来像"还没逾期"（未来方向仍用四舍五入）。「编辑服务器」页集中承载该服务器的全部配置：节点设置（名称/备注/**只读时区**，2026-09-16 修订：时区从「流量周期」区块上移——它约束的是全部按日切分的记账，不属于某一个周期）、流量（agent 上报的网卡下拉/统计模式/配额）、**独立的流量周期**（无/按月/按季/按年与秒级下次重置时间）、缴费周期（类型/长度/下次到期日/费用）、延迟测量端点选择、IP 列表（含手动主 IP）、sing-box 服务端配置（版本/启停/端口）。节点详情页不再承载配置表单与删除按钮——监控与配置分离，删除服务器统一走设置页（带确认）。**「添加节点」（注册 token + 安装命令弹窗）原先挂在概览页页头，现移到本页页头**：注册探针是接入/配置动作而非监控动作，概览页因此退回纯只读卡片墙；空态文案（`no_nodes` / `servers_empty`）同步指向新位置。**主 IP 列可一键复制（实现修订 2026-09-18）**：主 IP 列在地址右侧附一个复制图标按钮（`CopyIcon` + `.ip-copy-btn`，图标随 `.icon-btn` 的 `--text-dim` 保持低调、常显而非 hover 才显——触屏没有 hover），点击即复制该地址并弹出「IP 已复制」toast；剪贴板不可用（非 HTTPS 源，连 `copyText` 的 execCommand 兜底也失败）回「浏览器不允许自动复制」错误 toast，而不是静默无反应。主 IP 为空时仍只显示 `-`、不出按钮；该单元格因此也带上 `nowrap`（复制图标必须紧跟地址，不能让 IPv6 把它折到下一行）——窄屏代价与上文同：在卡片内横向滚动。**sing-box 启动/停止按钮合一（实现修订 2026-09-19）**：sing-box 卡片按钮行里并排的「启动」「停止」合并成一个切换按钮——动作跟观察态走（在跑显示「停止」，停着显示「启动」），理由与 AI 页发送/停止合一（§12.6 实现修订 2026-09-18c）同源：互斥的两个按钮总有一个禁用着，操作员会把当前禁用那个当成坏了、把启用那个当成"再来一次"。切换的判据是**发现态快照 `local.running`**（无上报时回退管理态 `status==='running'`）——管理态 status 没有"已停止"分支（`recordSingboxState` 只认 Running→running / LastError→degraded / 卸载确认→absent，"停了但没出错"的帧不匹配任何 case、保留旧值），拿它当切换依据会在停止后继续显示「停止」；发现态由启停命令自带的 nudge→scanLocal 即时刷新，是最接近现场的信号。重启与卸载按钮不动。
...
- **详情页「命令」卡片移除（实现修订 2026-09-16）**：面板不再提供对探针的任意 shell 下发入口——前端命令卡片与 `POST /api/nodes/{id}/commands` 路由一并删除。指令队列本体（`commands` 表、`enqueueCommand`、离线排队与幂等）保留：AI 执行（§12）与面板动作（sing-box 启停/重启）仍走同一条队列与审计；`GET /api/nodes/{id}/commands` 保留，供 AI 面板轮询执行结果。取舍：普通面板用户少一个"顺手敲 shell"的危险面，命令执行的入口收敛到 AI 确认流（§12.3）。**辨析（2026-09-19）**：终端页右栏的「常用命令」（§11）不与这条决策冲突——它不是服务端下发 shell 的入口，而是操作员自己的交互终端键盘注入（服务端只转发 `terminal_input` 帧、不经手执行），语义与手敲等价，故无审计、无确认流。
- **视觉语言重铸到品牌色板（实现修订 2026-09-18）**：此前 UI 的强调色是通用蓝（#3b6ef6）+ 通用绿（#16a34a），与品牌 logo（靛蓝砖 #465DDD + 薄荷绿脉搏线，"Server Pulse"）毫无关联。现在整套 token 锚定品牌：**靛蓝是操作者的声音**（动作/链接/选中/进度条填充，浅色 #465DDD、深色 #8598F8），**薄荷绿是机器的声音**（在线/存活/成功，浅色 #0E9F76、深色 #5CEBB2，≈logo 脉搏线的可读版），其余全部收敛——卡片 14px 圆角 + 两层浅影、进度条 5px、细滚动条、`:focus-visible` 键盘焦点环、`::selection` 靛蓝实底、mono 数据声部补 `tabular-nums`（指标跳动时数字不再抖宽）。动效只留三处且全部尊重 `prefers-reduced-motion`：**登录页的脉搏扫描线**（唯一的品牌时刻：暗底线 + 一段亮薄荷沿 ECG 线扫过，`Server Pulse` 副标题随之落位）、**在线状态点的呼吸光晕**（`dot-heartbeat` 2.8s——"在线"从静态圆点变成有心跳的东西）、toast 入场。配套细节：顶栏改磨砂玻璃（`--glass` + `backdrop-filter`，无支持时回退实底）、主按钮 hover 用具体色 `--accent-hover`（`filter:brightness` 会把靛蓝洗灰）、AI 气泡角色标签去全大写、终端屏幕色与 xterm canvas 主题色（不能读 CSS 变量）在 `styles.css` 与 `Terminal.tsx` 两处**手工保持同步**（#111927 / 边框 #2A3348 / 光标 #8598F8），`index.html` 补 `theme-color` 双 metas（浏览器外框色调跟随系统，与应用内切换互不影响）。不变量：未引任何外部字体/CDN（系统栈 + mono 双声部是刻意选择，CJK 面板引 Latin webfont 只会得到混排脸），全部布局类与既有陷阱注释（table-wrap、terminal-host、row-actions 等）原样保留。
- **同一个按钮类在 `<a>` 与 `<button>` 上不等高**（实现修订 2026-09-21）：UA 样式表给表单控件 `line-height: normal`，锚点则继承页面的 1.5；`.btn` 自己没有声明 `line-height`，于是同一个 `.btn.ghost.small` 的内容高是 21px（`<NavLink>`）对 ~16px（`EN`，纯 Latin 字形）对 ~18px（`跟随系统`/`退出登录`，CJK 字形）——**顶部工具条一行五个同类按钮量出 29.5 / 26 / 28 三种高度**，看起来就是"按钮没对齐"，但翻 CSS 找不到任何矛盾，因为差异根本不在样式表里而在元素类型与字形上。现在 `.btn` 显式 `line-height: 1.5`：保留锚点既有尺寸（37px 的页头按钮不变），把 `<button>` 抬到同一高度。`.icon-btn` 是同一根因的相反方向：`.icon-btn` 挂在 `<a>` 上时 `display` 仍是 `inline`，内联 SVG 骑在基线上把盒子撑到 27px，而旁边的 `<button>`（`inline-block`）是 24px——即"编辑/删除"图标按钮一高一矮；所以 `.icon-btn` 显式 `display: inline-flex; align-items: center; justify-content: center; min-height: 24px`（flex 容器没有文字 strut，不给 `min-height` 的话每个图标按钮都会矮 4px，13px 的 `ip-copy-btn` 更会把整行拉到 17px）。**判据：任何按钮类都必须自己声明 `display` 与 `line-height`**，不要让元素类型决定盒子。代价：`<button class="btn">` 会比改动前高几 px（正是这次要达到的一致），以及按钮类多两条需要跟着维护的声明。**（同日再修订：页头主按钮改 `tonal`——"看起来更高"不是高度问题）**：节点详情页头那一对改完仍被报告"高度不一致"。把用户截图逐像素量过：白底那只的描边纵向 y=39→112，蓝底那只的填充也是 y=39→112（74 设备像素 ÷ 2 = **37 CSS px**，与真实页面上量到的 37/37 一致）——**盒子逐像素相同**。差的是"边界的存在感"：实心 `--accent` 块的边是硬的，而描边款是 `--border`(#e3e6ee) 压在 `--bg`(#f3f4f9) 上（对比度约 1.1:1，几乎看不见），眼睛拿**可见边界**当尺寸依据，于是同尺寸下实心块被读得更大。**教训：这类报告不要先去改高度**——几何本来就一致，补几像素反而会把两只变成真的不等高（08-21 页头那对从头到尾都是 37/37）。做法：新增 `.btn.tonal`（`--accent-weak` 底 + `--accent` 描边与文字，hover 填充实心——与既有 `.btn.danger` 同一套 idiom），Web 终端按钮由 `primary` 换成 `tonal`：保住"主操作"的颜色信号，去掉压过邻座的实心块。**作用域只在这一对**；要全站把 `primary` 都换成淡底款是另一个决定（会一起改变保存/添加节点等页面的观感），要先说。**（同日晚些时候：`tonal` 已回退）**：淡底款只是把错觉挪到了描边上——它的描边是可见的 `--accent`，邻座的描边仍是几乎看不见的 `--border`，操作员照样读出不齐（"这种效果也会有错觉"）。故 `.btn.tonal` 删除、页头恢复 `.btn.primary` 实心款。**结论：这是观感问题而非 CSS 缺陷，不要在样式层反复试**——把几何量测（同 y、同 37px）摆给操作员，并保留既有的 primary/secondary 语言即可。
...
- **详情页 sing-box 运行状态卡片 + 延迟卡片上移（实现修订 2026-09-19）**：两处只动前端（`GET /api/nodes/{id}` 的 `singbox` 对象早已带 `status`/`last_error`，不新增接口）。① 基本信息网格在 sing-box 已启用（`desired_version` 非空，与「AnyTLS 端口」卡片同一条门槛）时，在端口卡片之前多一张 **sing-box 状态**卡片：值复用编辑服务器页状态块的 `sb_status_*` 文案（未安装/安装中/运行中/异常/已回滚，未知状态串回退原文），`last_error` 非空时作副标题——异常与回滚在详情页一眼可见，不必进编辑页找。② **延迟卡片从页面底部上移到「近 7 天指标」之前**，且只在节点勾选了延迟目标（`latency_targets` 非空）时渲染——没开启测量的节点不再收到一张指向"去新建目标"的空卡片；操作者最常看的延迟历史因此排在指标图前面。**（同日追加）状态卡片收编版本号**：sing-box 版本从「AnyTLS 端口」卡片的副标题移到状态卡片副标题（`last_error` 非空时以 ` · ` 追加在其后；`.tile-sub` 是 nowrap+ellipsis 单行，长错误截断显示不换行），端口卡片不再重复展示版本。**（同日追加）「sing-box 入站」表格**：基本信息卡片下方新增独立表格卡片，列出探针 config.json 里**全部**监听——多个 anytls、vless/socks 混合、one-sing.sh 接管的入站一视同仁（§17g 监听全部平等），不再是"只显示面板托管的那一个端口"；数据源是 `node_singbox_inbounds`（`GET /api/nodes/{id}` 新增 `singbox_inbounds` 字段，只含 port/type/tag/status，**无凭据**；发现型节点同样有行，纯旧版 agent 未上报过 config 时为空、整卡不渲染）。列文案复用编辑页的 `sb_col_*`，状态复用 `sb_inbound_running/pending/deleting`。详情页只读，编辑仍走「设置 → 服务器 → 编辑」。**（同日追加）详情页头部「编辑」按钮**：web 终端按钮左侧新增 `.btn`（PencilIcon + `edit` 文案），直达 `/settings/servers/{id}`——配置入口在设置页的收敛（2026-09-15）不变，这里只是一条跳转捷径，不再让操作者为改个名字穿过两级导航。**（同日追加）概览卡片 IP 一键复制**：卡片 IP 右侧附与设置→服务器列表同款的 `ip-copy-btn`（常显、`ip_copied`/`copy_manual` toast 同文案）——同一份地址在两处的交互保持一致。两条实现细节：卡片本体是 `<Link>`，复制按钮的 onClick 必须 `preventDefault` + `stopPropagation`，否则点复制会顺带跳进详情页；toast 节点渲染在 Link 外（fixed 定位位置无关，但交互节点不嵌进锚点）。**（同日再修订：AnyTLS 端口卡片删除、入站表格沉底、IP 行对齐）**：① 入站表格就位后「AnyTLS 端口」小卡片没有存在价值——它只承载面板托管的那一个端口，而表格已经把每个监听一行行列全（多 anytls/vless/socks 混合不再只露一个口），卡片连同 `sb_anytls_port` 文案一并删除，`node_singbox.port` 列本身不动（卸载/重装回圈与订阅兜底仍读它）。② 入站表格从基本信息下方移到**页面最底部**（流量卡片之后）：它是参考资料而非一眼要看的监控，不该把延迟/指标往下顶。③ 卡片 IP 行的复制按钮错位修复：`.ip-copy-btn` 的 `vertical-align: middle` 是对齐到文字基线 + 半个 x 高度，视觉上按钮浮起；`.node-card-ip` 改 flex + `align-items: center`（flex 上下文里 vertical-align 失效，按钮落在 IP 的竖直中线上），地址包进 `.ip-text`（overflow:hidden 顺带把长 IPv6 变省略号、不把按钮顶出卡片）——设置→服务器列表那处按钮在表格单元格里没这个观感问题，不动。**（2026-09-21 追加：头部两个动作按钮等高、都带图标）**：这一对按钮此前形状不一致（编辑带 `PencilIcon`、Web 终端是纯文字），页头里读起来不齐。现在 Web 终端补自绘 `TerminalIcon`（仍是内联 SVG、不引图标库与 CDN），并给这一行的容器单独标 `.head-actions`：`align-items: stretch` 让两个按钮共用一个高度——**用拉伸而不是硬编码高度**，高度随字体与内边距走，将来改 `.btn` 的 padding 不会留下一个对不上的 magic number；文案加 `nowrap`，被挤窄时不会其中一个折成两行、另一个仍是单行；`.btn-icon` 补 `flex: none`，图标不再被 flex 压缩变形（图标按钮在窄容器里被压扁的根因）。
...
- **详情页端口转发列表（实现修订 2026-09-21，§21）**：节点详情页底部、紧接「sing-box 入站」表格之后新增同款只读表格卡片，列出探针最近上报的全部 nftables DNAT 转发（§21）——含 nfpf.sh 与手工 `nft` 加的。读取与 §21 卡片**逐行同源**（`forwardViews` 一处映射、两个接口共用），所以两侧不可能对同一条规则给出不同读法。数据源是 `GET /api/nodes/{id}` 新增的 `forwards` 数组（`node_forwards` 快照，**不发起实况往返**：打开页面绝不阻塞探针，探针离线时照旧显示最后一次上报）。列为协议/源端口/入接口/目标/备注，文案复用编辑页的 `fw_col_*`；带面板不建模匹配条件的规则仍显示「含额外条件」标签。空数组（旧 agent 从未上报 §21，或探针本来没有转发）整卡不渲染。详情页只读——增删改仍走「设置 → 服务器 → 编辑」。

- **审计日志独立成页（2026-09-16）**：审计从基础设置页的卡片移出，成为设置的子页 `/settings/audit`（子导航入口「审计日志」），表格结构与 `GET /api/audit` 的其余字段不变。节点列显示节点名称而非 ID：`ListAudit` 读取时 `LEFT JOIN nodes` 带出 `node_name`（`COALESCE` 成空串，避免无节点记录的 NULL 扫描错误），改名后的历史记录显示当前名称，节点已删除时前端回退显示原始 ID；审计表本身不回写，无 schema 变更。**（实现修订 2026-09-19：标准页码分页）** 这一页此前只拉最新 200 条，更早的记录在面板上**没有任何入口**。现在做**标准页码分页**：默认每页 20 条，底部下拉可切 20/50/100/200（选择记在 localStorage），并给出总数、「第 x / y 页」、首/中/末页码按钮（间隔折叠成省略号）与「跳至第 N 页」输入框；`GET /api/audit?limit=&page=` 返回 `{entries, total, page, limit}`。越界页码在服务端**收敛**到最后一页并在响应里回显实际页——否则改小每页条数、或审计表继续增长之后，操作员手里的页码会指向一个空表；每条 entry 带 `id` 作 React key。**代价（说清楚：这是本页明确选定并接受的）**：页码与跳页必须有 `COUNT(*)` + `OFFSET`，而这张表从不清理（§4.4）——① 行数越多，计数与深翻页越慢（扫 PK 索引 + 跳过 offset 行）；② 更实质的是**窗口不稳**：审计行随时在追加（心跳跃迁、通知重试），新行插在列表顶部，翻页时可能重复或漏掉一两行。keyset 游标（`before=<上一页末条 id>`）能消掉第 ② 条，但它给不出页码与跳页——"标准分页"这个交互本身就标着这个价码，要用游标就得推翻它。
- **AI 设置独立成页（2026-09-18，§12）**：AI 从基础设置页的卡片升级为子页 `/settings/ai`（子导航入口「AI 助手」），承载：provider 列表（增删改 / 协议 / root `base_url` / 密钥 / 额外请求头 / models.dev slug / 启用开关 / 「拉取模型列表」）、模型列表（拉取结果是可搜索列表、标注与 models.dev 的匹配状态、勾选批量添加、逐字段手改、启用开关）、默认模型（按 provider 分组的选择器）、`ai.default_policy`（思考预算与 Kill Switch 都不再有界面，见 §12.5 / §12.3）。基础设置页删除 AI 卡片、`ai.base_url` / `ai.model` / `ai.api_key` 三键与相应 i18n 文案。终端页发送框同步新增：按 provider 分组的模型下拉、思考档位下拉（含 `off`，按模型能力裁剪）、「停止」按钮、思考折叠面板（默认收起）。**（实现修订 2026-09-18c）两个下拉移进发送行**（与发送按钮同一行，模型占余量、档位定宽、窄面板 `flex-wrap` 换行），**发送与停止合并成一个按钮**：二者互斥（回合进行中没有可发送的草稿，空闲时也没有可停止的回合），原先同一位置摆两个按钮（其中一个禁用）会让操作员在回合进行中把「停止」点成「发送」——按钮 `type` 随角色切换（busy 时 `button` + 中止，空闲时 `submit`），所以忙碌态永远不会误提交草稿。「设为默认」随模型下拉留在同一行末位（唯一可能换行的一项）。**（实现修订 2026-09-18d）发送行右侧的「设为默认」按钮删除**：默认模型只在「设置 → AI 助手」页改，面板不再承担配置写入口（`ai_default_unavailable` 提示保留，改成指引去设置页）。**侧栏头部改为一个「+」新建会话按钮**，原「会话继续 / 新会话」chip 删除——它只复述 transcript 已经摆出来的事实，而「重新开一段」反而没有入口；新建会话清空面板 transcript 与 sessionStorage 的会话 id（**服务端旧会话一行不删**），进行中的回合先以 `discard` 静默中止，否则半截流会追加进新记录、读起来像模型在答一个没人问的问题。**侧栏高度从 `min-height` 改为定高 `min(720px, calc(100vh - 180px))`（窄屏下限 420px）**：定高之前 `.ai-messages` 的 `flex:1 + overflow-y:auto` 拿不到解析后的高度，于是气泡每多一条就把网格行撑高，而同行的终端卡片跟着一起长高、整页被顶长——列表永远不出现纵向滚动条。定高后列表成为滚动容器，并**自动锁底**：新气泡提交后滚到末尾，操作员上滚超过 48px 即解除跟随（再滚回底部自动重新锁定），不会把正在回读的人拽走。**（实现修订 2026-09-18e，用户拍板减负）三处面板噪音删除 + 一处输入习惯**：① 助手消息区上方那个「节点 / 终端会话」上下文框整个删除——节点 id 与会话 id 都是终端卡片头部已经摆出来的事实，复读一遍只占高度；② 「本轮完成」类横幅不再出现（见 §12.6）；③ 默认模型卡从「选择 (provider, 模型) 对」扩成**三件套**，新增「默认思考档位」下拉（`ai_reasoning_default` 复用面板的「默认(不指定)」作为未设置项；设置了才显示解释性 hint），语义见 §12.1；④ **发送框 Enter 即发送**，Shift+Enter 换行；`isComposing` 守卫是硬要求——中文输入法确认候选词的 Enter 也是 keydown Enter，没有守卫每打一个词就会把半截草稿发出去。**（实现修订 2026-09-18f）transcript 改成「一轮一气泡 + Markdown 实时渲染」**：此前一条 entry 一个气泡、正文按纯文本 `pre-wrap` 渲染，于是模型一轮内的「思考 → 说话 → 跑命令 → 再说」被画成三四个并列气泡，读起来像几个助手在轮流发言，模型输出的 Markdown（列表、表格、代码块）也摊成符号。现在 ① **每个用户消息开一个气泡，它之后到下一个用户消息之前的全部内容——思考块、正文、工具卡、工具结果、回合结束原因——装进同一个 assistant 气泡**：分组是渲染期的一次纯函数（`web/src/components/aiTranscript.ts` 的 `groupTurns`），入口仍是那条扁平 entry 列表（循环可以在一轮内交错 text/tool，而服务端不送「这条工具结果是哪次助手轮问的」，嵌套模型只能凭空发明分组）；`AssistantEntry = Exclude<ChatEntry, { kind: 'user' }>` 把「气泡体里不可能有用户消息」写进类型，渲染端的兜底分支因此能确定自己是确认框。② **助手正文走 Markdown**（`web/src/components/Markdown.tsx`：`react-markdown` + `remark-gfm`），每来一个 delta 重解析一次，所以是流式实时渲染。三条硬边界：**不引 rehype-raw** ⇒ 模型写进正文的 HTML 只能是文本（实测 `<img src=x onerror=…>` 原样显示、`window.__xss` 未被赋值），`javascript:` 链接被 react-markdown 的默认 `urlTransform` 过滤；**图片一律不加载**——Markdown 图片等于让模型替面板发起一次第三方请求（追踪像素 + 泄露面板地址），`img` 节点只渲染成占位文字（`ai_image_blocked`），与 fog 的聊天气泡同一条决定；**气泡内的嵌套块改用 `--panel`**（工具卡 / 确认框现在长在 assistant 气泡里，同用 `--panel-2` 就与气泡同色、只剩一圈边框）。底色按「谁在说话」分开：用户消息用 `--accent-weak` + 3px accent 内阴影（10% 的色差在低对比屏上不足以区分，竖条才是真正的区分），助手保持中性面；问题与回答之间的间距小于轮次之间的间距，一对问答读作一次交换。**定高 + 自动锁底那两条规则不变**（见本节 2026-09-18d）——分组只改渲染结构，滚动容器仍是 `.ai-messages`；Markdown 会显著改变气泡高度，锁底因此更重要。代价：① `react-markdown` + `remark-gfm` 带进 lockfile 97 个包，打包后 min ≈153 KB / gzip ≈47 KB（用户气泡仍按纯文本渲染——操作员打的是句子，当 Markdown 解析会悄悄重排它恰好含有的符号）；② 中途的话与工具卡共处一个气泡，找某条命令要多滚一点（换来的是「这一轮到底做了什么」一眼可见）。**实测**（无头 Chromium，真实 `styles.css` + 真实 Markdown 组件，420px 视口 / 338px 面板）：2 轮 → 4 个气泡（用户 / 助手各 2），首轮 5 个 body 块同处一个助手气泡；两个底色分别为 `rgba(59,110,246,.1)` 与 `rgb(247,249,252)`；页面与气泡横向溢出均为 0，`pre` 在气泡内滚动；内容超过视口时 `.ai-messages` 仍滚到真底（`scrollTop == scrollHeight - clientHeight`）。
- **设置 → 常用命令独立成页（2026-09-19，§11）**：子页 `/settings/commands`（子导航入口「常用命令」），承载终端页右栏命令片段的增删改与上移/下移排序（整体重排一个接口）。每条 = 名称 + 命令（textarea，多行）；编辑走 Modal，删除二次确认。数据与校验规则见 §11；面板侧的 `norm*` 白名单照例三步走（`types.ts` → `api.ts` norm → 页面）。
...
- **图表图例即开关（实现修订 2026-09-18）**：详情页五张图（CPU / 内存·磁盘 / 网络速率 / 每日流量 / 延迟历史）的图例条目改成按钮，点一下隐藏该指标、再点恢复，`LineChart.tsx` 与 `BarChart.tsx` 共用同一套行为（`aria-pressed` 表示当前是否可见，`title` 给出提示文案）。三条口径：① **y 轴只按可见序列重算**——不这样做，隐藏一条高值序列（如网络的 tx 远大于 rx）后剩下的曲线仍被压在底部，看不清；② 隐藏态按 **series 名**记录，与图例/ tooltip 行的身份一致（重名延迟目标靠 `NodeDetail.tsx` 的 `(kind)` 后缀区分），状态只活在该图实例内：30s 轮询刷新不丢，但各图彼此独立（网络图的 rx 与流量柱的 rx 是两处状态）；③ **全部隐藏时不进空态**——坐标轴、网格与图例照旧渲染，只在图中央覆盖一行提示。第 ③ 条是必须的：空态（`no_chart_data`）会把整张图连同图例一起卸载，而图例是唯一能把序列点回来的控件，进空态就是死路。隐藏的序列也不进 tooltip（提示框读作"这张图现在画的是谁"）。代价：隐藏态以名称为键，切换语言或重命名延迟目标后旧键失配，被隐藏的序列会重新出现（再点一次即可）——可接受，不值得为此给 `ChartSeries` 引入稳定 id。文案 `chart_legend_toggle` / `chart_legend_all_hidden` 走 i18n（zh/en 两份）。
- 节点卡片展示：名称、国旗、IP、CPU%、内存%、磁盘%、CPU 核数、流量使用率（按所选模式）、今日上下行、**费用标签**（2026-09-16 新增）、到期倒计时、延迟摘要。费用的取值就是缴费周期里那行自由文本（`node_billing.note`，随 `GET /api/nodes` 的 `billing_note` 一起下发，省掉逐节点详情的额外请求），非空即贴，位置固定在卡片头、发行版徽标左侧（“这台多少钱”挨着“这台是什么”），用 accent 色与状态色区分；不解析币种与周期，也不要求缴费周期已配置——它是操作者写给自己看的一行账。**连接状态改由卡片自身样式表达（实现修订 2026-09-16）**：卡片头右侧的位置改为发行版徽标（复用详情页的自绘 SVG，未知/未上报 ID 回退首字母徽标），不再画在线状态小圆点；离线卡片红边框 + 底色红色斜纹（`--red-weak`，明暗主题各有一档），状态文字保留在卡片 `title` 提示里。**到期倒计时对过期节点直接贴「已过期」标签（实现修订 2026-09-17）**：到期日存本地零点，过期之后 `Math.ceil(负的零点几)` 得到 `-0`，而 `-0 >= 0` 为 true，卡片曾把已过期的节点显示成「0 天后到期」；现在直接比较时间戳，非未来一律「已过期」（红色 chip，未过期仍是 amber 倒计时）。设置→服务器列表的到期列本来就是「已逾期 N天」（2026-09-21 起同样只印整天数），不受影响。
...
- **静态资源缓存策略**（实现修订 2026-09-21）：静态处理器此前对 `index.html` 与 `/assets/*` **都不发 `Cache-Control`**（只有 `Last-Modified`），浏览器于是按**启发式新鲜度**（Chrome 约为「Last-Modified 距今」的 10%）自行决定复用多久——在「上一次构建是十几小时前、刚刚重建镜像」的部署里，这个窗口可以长达一两个小时，于是**刷新页面看到的仍是上一版面板**：页面本身一切正常，只是 UI 是旧的，排查方向被整体带偏到「改动没生效/没重新构建/没重启」。现在分成两类：`/assets/*` 是 Vite 的内容寻址产物（文件名随字节变）⇒ `public, max-age=31536000, immutable`；`index.html`、它的 SPA 兜底、以及根目录未哈希资源（`logo.svg`）⇒ `no-cache`，每次加载都回源校验（未变时仍是廉价的 304，所以选 `no-cache` 而不是 `no-store`）。**同一条兜底的另一处修正**：缺失的 `/assets/*` 现在回 `404 unknown_asset`，不再落进 SPA 兜底吐 `200 text/html`——旧行为让「旧 shell 请求已不存在的 bundle」在控制台表现为模块解析错误（把 HTML 当 JS 执行），而不是一个干净的 404。代价：每次冷加载多一次条件请求；静态资源、`/dl`、`/install.sh` 自此是三套缓存策略，改静态处理器时要一起看。**（同日再修订：`no-cache` 不够，shell 改按内容校验）**：`no-cache` 依赖回源校验，而它依赖的 `Last-Modified` **在这个镜像里不可信**——`/srv/web` 下所有文件带着**同一个 mtime**（首次创建该路径的那一层的时间戳），重建后内容变了、时间戳不变，于是浏览器回源拿到的是 **304**，磁盘上明明是新的、屏幕上还是旧的：同一天内"回退 tonal 却仍看到淡底款"就是这么来的（服务器磁盘上已 0 处 `tonal`，头部却回 304）。现在 shell（`index.html`、SPA 兜底、未哈希根资源）改为**内容 ETag** 校验，并以**零 modtime** 交给 `ServeContent` —— 于是既不输出 `Last-Modified`、也**故意忽略 `If-Modified-Since`**：手里只有旧 `Last-Modified` 的浏览器会拿到一个完整的 200（自愈），而已经拿到 ETag 的浏览器在内容未变时仍是廉价的 304。`/assets/*` 不变（内容寻址 + `immutable`，它的 `Last-Modified` 也用不着）。判据：**任何依赖文件 mtime 的缓存逻辑，在镜像/跨文件系统拷贝场景下都要当成坏的**。
