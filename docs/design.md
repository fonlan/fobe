...
> - **凭据与证书都按文件渲染**：`LiveProxyNodes` 不再把面板的全局 anytls 密码写到那个端口上——文件是探针此刻真正在服务的东西，脚本创建的监听必须继续用它原来的密码，否则订阅会发出一条认证不了的 URI。面板的全局密码只在**面板自己写那个监听**时（安装 / 编辑）进入文件，此后文件带着它，渲染照旧（面板装的节点行为不变）。证书同理：文件里该端口的 PEM 优先，`node_singbox.cert_pem` 只在文件没给时兜底——探针换过证书之后，旧的那份不能再被 pin。
