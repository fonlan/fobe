...
> - **重名只在需要时加端口**：节点有 **≥2 条可渲染入站**时，直连自动名 = 基础名 + `:` + 端口（`东京:8443`）；只有一条时**保持原名不加后缀**（17j 的结论不变：绝大多数节点只有一条入站，`nodes.sub_name` 必须逐字生效）。显式 alias 依旧逐字优先（两个同名 alias 仍走渲染器 `-2`/`#2` 兜底，保存时不因"同节点"而报 `alias_conflict`——它们现在是两行，各自可改名）。**代价（说清楚）**：多条入站的节点，客户端看到的 tag/name 会从 `B`/`B-2` 变成 `B:8443`/`B:8444`，pin 过旧 tag 的模板要重拉订阅（同 §10 实现修订 2026-09-16 对 tag 的取舍）。
